...
import com.nidoham.kson.annotation.JsonAdapter
...
data class FieldInfo(
    ...
    fun getAllNames(): List<String> {
        return if (alternateNames.isEmpty()) listOf(serializedName ?: name) else listOf(serializedName ?: name) + alternateNames
    }
}
...
data class ClassInfo(
    ...
    fun getField(name: String): FieldInfo? { return fieldMap[name] }
    fun getFieldBySerializedName(name: String): FieldInfo? { return serializedFieldMap[name] }

    fun findField(name: String): FieldInfo? {
        fieldMap[name]?.let { return it }
        serializedFieldMap[name]?.let { return it }
        for (field in fields) {
            if (name in field.alternateNames) return field
        }
        return null
    }
}