...
class JsonWriter(
    ...
    fun write(element: JsonElement): String {
        ...
        val result = buildString { writeElement(element, this, 0) }
    ...
    private fun writeObject(obj: JsonObject, sb: StringBuilder, depth: Int) {
        ...
        entries.forEachIndexed { index, (key, value) ->
            ...
            sb.append('"'); sb.append(escapeString(key)); sb.append('"'); sb.append(':')
            if (prettyPrint) sb.append(' ')
    ...
    private fun writePrimitive(primitive: JsonPrimitive, sb: StringBuilder) {
        ...
        when (value) {
            null -> sb.append("null")
            is String -> { sb.append('"'); sb.append(escapeString(value)); sb.append('"') }
            is Boolean -> sb.append(value)
            is Number -> sb.append(formatNumber(value))
            else -> { sb.append('"'); sb.append(escapeString(value.toString())); sb.append('"') }
        }
    }
    ...
    fun setPrettyPrint(prettyPrint: Boolean): JsonWriter { this.prettyPrint = prettyPrint; return this }
    fun setIndent(indent: String): JsonWriter { this.indent = indent; return this }
    fun setSerializeNulls(serializeNulls: Boolean): JsonWriter { this.serializeNulls = serializeNulls; return this }
    fun setEscapeHtmlChars(escapeHtmlChars: Boolean): JsonWriter { this.escapeHtmlChars = escapeHtmlChars; return this }

    companion object {
        @JvmStatic fun compact(): JsonWriter = JsonWriter(prettyPrint = false)
        @JvmStatic fun pretty(): JsonWriter = JsonWriter(prettyPrint = true)
    }
}