...
class DateAdapter(
    ...
    private val formatter: SimpleDateFormat by lazy {
        SimpleDateFormat(dateFormat, Locale.US).apply {
            this.timeZone = this@DateAdapter.timeZone
            this.isLenient = this@DateAdapter.lenient
        }
    }

    override fun serialize(value: Date): JsonElement {
        return if (serializeAsTimestamp) JsonPrimitive.of(value.time) else synchronized(formatter) { JsonPrimitive.of(formatter.format(value)) }
    }

    override fun deserialize(element: JsonElement): Date? {
        if (element.isJsonNull) return null
        return if (element.isJsonPrimitive && element.isNumber) {
            Date(element.asLong())
            ...
            synchronized(formatter) {
                try {
                    ...
                    logger.error("Failed to parse date: ${element.asString()}")
                    if (lenient) tryParseAlternativeFormats(element.asString()) else throw IllegalArgumentException("Failed to parse date: '${element.asString()}' with format: $dateFormat", e)
                }
            }
        } else {
            throw IllegalArgumentException("Cannot parse Date from: $element")
        }
    }

    override fun nullValue(): Date? {
        return null
    }
    ...
    private fun tryParseAlternativeFormats(dateString: String): Date? {
        ...
        for (format in formats) try {
            return SimpleDateFormat(format, Locale.US).apply { timeZone = this@DateAdapter.timeZone }.parse(dateString)
        } catch (e: Exception) { }
    ...
    companion object {
        ...
        @JvmStatic fun iso8601(): DateAdapter { return DateAdapter(ISO_8601) }
        @JvmStatic fun timestamp(): DateAdapter { return DateAdapter(serializeAsTimestamp = true) }
    }
}